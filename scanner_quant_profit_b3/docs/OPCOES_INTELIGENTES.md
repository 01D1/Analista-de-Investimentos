...
## Integração com Inteligência Integrada

A Fase 24 usa `option_structure_candidates` e `option_walk_forward_runs` como componente opcional do snapshot por ativo. Se não houver opções, se houver bloqueio por liquidez ou se a validação OOS estiver insuficiente, o componente de opções reduz confiança e não gera chamada operacional.
