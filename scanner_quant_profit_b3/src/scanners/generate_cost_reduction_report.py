"""Gerar relatório Markdown de redução de custos salvo."""
...
from src.paper.cost_reduction_store import load_cost_reduction_results, load_cost_reduction_runs
...
def run(run_id: int | None = None, output: str | Path | None = None, db_path: str | Path | None = None) -> dict:
    ...
    runs = load_cost_reduction_runs(db)
    if runs.empty:
        results = load_cost_reduction_results(db, run_id=run_id)
        base_id = 0
    else:
        ...
        base_id = int(selected.get("base_paper_run_id") or 0)
        results = load_cost_reduction_results(db, run_id=run_id)
    out = Path(output) if output else project_path("data/reports") / f"cost_reduction_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    path = save_cost_reduction_report(out, base_id, results)
    return {"run_id": run_id, "report_path": str(path)}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Gerar relatório de redução de custos. Não recomendação.")
